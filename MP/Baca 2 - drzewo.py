    ...
    def print_inorder(self):
        def inorder(node):
            if node is None:
                return
            inorder(node.left)
            print "Key: %s, Count: %s" % (node.key, node.count)
            inorder(node.right)
